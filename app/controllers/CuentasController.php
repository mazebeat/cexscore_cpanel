...
class CuentasController extends \ApiController
{
	...
	public function store()
	{
		...
		$messages = array(
			...
			'cliente.id_pais.required'                => 'El campo pais' . $required,
	...
	public function edit($id)
	{
		...
		$idregion = null;
		if (!is_null($cliente->id_pais) && !is_null($cliente->id_ciudad)) {
			$region = Region::find(Ciudad::find($cliente->id_ciudad)->id_region);
			if (!is_null($region) || $region->exists()) {
...
//		dd($cliente);
		...
		return View::make('admin.cuentas.edit', compact('cliente'))
		           ->with('states', $states)
		           ->with('plans', $plans)
		           ...
		           ->with('catgs', $catgs)
		           ->with('momentoencuestum', $momentos)
